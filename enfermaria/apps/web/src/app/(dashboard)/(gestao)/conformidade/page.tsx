...
export default function ConformidadePage() {
  ...
  const { data: checklistData } = useQuery<ChecklistItem[]>({
    queryKey: ['conformidade-checklist'],
    queryFn: () => api.get('/audit/checklist').then(r => r.data),
    staleTime: 30_000,
  });

  const checkStates: Record<string, CheckState> = Object.fromEntries(
    (checklistData ?? []).map(i => [i.itemKey, i.estado as CheckState])
  );

  const mutation = useMutation({
    mutationFn: ({ itemKey, estado }: { itemKey: string; estado: CheckState }) =>
      api.patch(`/audit/checklist/${itemKey}`, { estado }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['conformidade-checklist'] }),
  });

  const toggleCheck = (id: string) => {
    const estados: CheckState[] = ['verificar', 'conforme', 'nao_conforme'];
    const atual = checkStates[id] ?? 'verificar';
    const idx = estados.indexOf(atual);
    const next = estados[(idx + 1) % 3];
    mutation.mutate({ itemKey: id, estado: next });
  };
  ...
  const conformes  = (checklistData ?? []).filter(i => i.estado === 'conforme').length;
  const naConforme = (checklistData ?? []).filter(i => i.estado === 'nao_conforme').length;
                ...
                <div key={cat} className="bg-white rounded-2xl border border-slate-100 shadow-sm" style={{ padding: '20px 24px' }}>
                  ...
                  <div className="flex flex-col gap-2">
                    {CHECKLIST.filter(c => c.categoria === cat).map(c => {
                      const estado = checkStates[c.id] ?? 'verificar';
                      const itemData = (checklistData ?? []).find(i => i.itemKey === c.id);
                      return (
                        <div key={c.id} className="flex items-center justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-slate-700">{c.label}</p>
                            {itemData?.atualizadoEm && (
                              <p className="text-xs text-slate-400" style={{ marginTop: '2px' }}>
                                Actualizado {new Date(itemData.atualizadoEm).toLocaleDateString('pt-PT')}
                              </p>
                            )}
                          </div>
                          <button onClick={() => toggleCheck(c.id)} disabled={mutation.isPending}
                            className={`shrink-0 text-xs font-semibold badge-pad py-1 rounded-full border transition-colors disabled:opacity-60 ${CHECK_COR[estado]}`}>
                            {CHECK_LABEL[estado]}
                          </button>
                        </div>
                      );
