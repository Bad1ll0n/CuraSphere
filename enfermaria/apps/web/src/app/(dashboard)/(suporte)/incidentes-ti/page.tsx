...
interface Nota {
  id: string; conteudo: string; criadaEm: string;
...
export default function IncidentesTIPage() {
  ...
  const [adicionandoNota, setAdicionandoNota] = useState<string | null>(null);
  ...
  const adicionarNota = async (id: string) => {
    const texto = notaTexto[id]?.trim();
    if (!texto) return;
    setAdicionandoNota(id);
    try {
      const { data } = await api.post(`/incidentes-ti/${id}/notas`, { conteudo: texto });
      setNotas(prev => ({ ...prev, [id]: [...(prev[id] ?? []), data] }));
      setNotaTexto(prev => ({ ...prev, [id]: '' }));
    } catch { /* silencioso */ }
    finally { setAdicionandoNota(null); }
  };
                      ...
                      <div className="col-span-2">
                        ...
                        <div style={{ marginTop: '20px' }}>
                          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide" style={{ marginBottom: '10px' }}>
                            Notas ({(notas[inc.id] ?? []).length})
                          </p>
                          <div className="flex flex-col gap-2" style={{ marginBottom: '10px' }}>
                            {(notas[inc.id] ?? []).length === 0 ? (
                              <p className="text-xs text-slate-400 italic">Sem notas ainda.</p>
                            ) : (notas[inc.id] ?? []).map(n => (
                              <div key={n.id} className="bg-white rounded-xl border border-slate-200" style={{ padding: '10px 14px' }}>
                                <div className="flex items-center justify-between" style={{ marginBottom: '4px' }}>
                                  <span className="text-xs font-semibold text-slate-700">{n.autor.nome}</span>
                                  <span className="text-xs text-slate-400">{new Date(n.criadaEm).toLocaleDateString('pt-PT', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</span>
                                </div>
                                <p className="text-sm text-slate-600 leading-relaxed">{n.conteudo}</p>
                              </div>
                            ))}
                          </div>
                          <div className="flex gap-2">
                            <textarea
                              value={notaTexto[inc.id] ?? ''}
                              onChange={e => setNotaTexto(prev => ({ ...prev, [inc.id]: e.target.value }))}
                              placeholder="Adicionar nota ou comentário..."
                              rows={2}
                              className="flex-1 border border-slate-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                              style={{ padding: '8px 12px' }}
                            />
                            <button
                              onClick={() => adicionarNota(inc.id)}
                              disabled={adicionandoNota === inc.id || !notaTexto[inc.id]?.trim()}
                              className="self-end bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-xl transition-colors disabled:opacity-50"
                              style={{ padding: '8px 14px', whiteSpace: 'nowrap' }}
                            >
                              {adicionandoNota === inc.id ? '...' : 'Adicionar'}
                            </button>
                          </div>
                        </div>
