 ...
 *   ATUALIZAR_BASELINE=1 pnpm nx run @org/mobile:test --skip-nx-cache
 */
import { readdirSync, readFileSync, statSync, writeFileSync } from 'fs';
import { join, relative } from 'path';

const RAIZ = join(__dirname, '..', 'screens');
...
function ecras(dir: string): string[] {
  return readdirSync(dir, { withFileTypes: true }).flatMap((e) => {
    ...
    return e.name.endsWith('Screen.tsx') ? [p] : [];
  });
}
...
describe('estrutura mobile - erro de carga vs lista vazia', () => {
